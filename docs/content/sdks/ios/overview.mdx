...
description: Native iOS SDK for ThunderID authentication in Swift applications
---
...
- **`ThunderSwiftUI`** — Drop-in SwiftUI components built on top of `ThunderID`
...
<CodeGroup>
  <CodeBlock lang="text" label="Swift Package Manager">
    ...
    Select the ThunderSwiftUI product (includes ThunderID automatically).
  </CodeBlock>
...
To get started quickly, follow the [iOS Quickstart Guide](/docs/next/guides/getting-started/connect-your-application/ios) for step-by-step setup instructions.
...
| Type | Description |
|------|-------------|
| `ThunderState` | Reactive `ObservableObject` injected by `.thunderIDProvider(config:)` |
...
- Override localized strings using `ThunderI18n` with your own translation bundles
