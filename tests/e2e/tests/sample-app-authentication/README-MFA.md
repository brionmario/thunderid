...
The tests use a mock SMS server to capture OTP messages sent by ThunderID, eliminating the need for real SMS providers during testing.
...
1. ✅ **ThunderID server running** at `https://localhost:8090`
2. ✅ **Sample app running** (e.g., at `https://localhost:3000`)
3. ✅ **Application ID** - Get from ThunderID setup
...
5. ✅ **MFA authentication flow configured** in ThunderID
6. ✅ **Test user with mobile number** created in ThunderID
7. ✅ **Application created with MFA flow attached** in ThunderID
...
### Option 2: Manual Setup (ThunderID Configuration)

If you prefer manual configuration or need to troubleshoot, follow these steps:

#### Admin Access Token

Run the following command, replacing `<application_id>` with your sample app ID (created during ThunderID setup).
...
Follow these steps:
1. Fetch all applications from ThunderID
...
| `SAMPLE_APP_ID` | Yes* | - | Application ID in ThunderID (*required for automated setup) |
| `SERVER_URL` | No | `https://localhost:8090` | URL of ThunderID server |
| `ADMIN_USERNAME` | No | `admin` | Admin username for ThunderID |
| `ADMIN_PASSWORD` | No | `admin` | Admin password for ThunderID |
...
2. Get application ID from ThunderID:
   ```bash
   curl -k 'https://localhost:8090/applications' \
...
**Solutions**:
1. Check ThunderID server logs for detailed error messages
...
4. Try manual cleanup and re-run:
   ```bash
   # Delete existing resources via ThunderID API
   # Then re-run tests
   ```
...
**If Persistent**: Manually delete resources via ThunderID API if needed.
...
2. Check ThunderID server logs for SMS sending errors
...
4. Check ThunderID server logs for OTP verification errors
...
**Solutions**:
1. Verify test user exists in ThunderID
...
4. **Verify ThunderID configuration** matches test expectations
...
- [ThunderID MFA Setup Guide](../../../../docs/guides/authentication/GUIDE-SMS-OTP-MFA-LOGIN.md)
- [Sample App Documentation](../../../samples/apps/README.md)
- [Playwright Documentation](https://playwright.dev/docs/intro)
- [ThunderID Flow Management API](../../../../docs/guides/flows/)

## Support

For issues or questions:
1. Check ThunderID server logs: `tail -f backend/server.log`
