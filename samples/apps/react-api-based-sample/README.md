...
This sample application demonstrates how to integrate ThunderID authentication into a React application using direct API calls instead of SDK-based OAuth redirects. It showcases API-based user registration (sign-up) and authentication (sign-in) flows.

## Features

- User registration via ThunderID's User Management API
- User authentication via ThunderID's Credentials Authentication API
...
- ThunderID server configured with appropriate CORS settings
...
Edit `public/config.json` with your ThunderID server settings:
...
The application runs on HTTPS. Copy the SSL certificates from your ThunderID distribution:
...
To use the sign-up functionality, you need to temporarily disable ThunderID security by setting the following environment variable before starting the ThunderID server:
...
├── utils/
│   ├── api.ts               # ThunderID API utilities
...
This sample interacts with the following ThunderID APIs:
...
- Check CORS configuration in ThunderID's `deployment.yaml`
...
- Ensure `SKIP_SECURITY=true` is set when starting the ThunderID server

**Issue**: CORS errors
- Add your application URL to "Allowed Origins" in ThunderID's configuration:
  ```yaml
  cors:
    allowed_origins:
