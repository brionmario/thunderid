...
This sample application demonstrates how to integrate ThunderID authentication into a React application using the `@asgardeo/react` SDK. It showcases OAuth 2.0/OIDC based user authentication, token management, and user profile display.

## Features

- 🔐 OAuth 2.0/OIDC authentication with ThunderID
...
- An OAuth application registered in ThunderID with appropriate redirect URIs
...
Open `dist/runtime.json` and set your ThunderID application credentials:
...
|----------|-------------|
| `clientId` | The OAuth client ID from your ThunderID application |
| `baseUrl` | The base URL of your ThunderID server |
...
For HTTPS support during development, copy the SSL certificates from your ThunderID distribution to the project root:

```bash
# From ThunderID distribution
...
```json
{
  ...
  "baseUrl": "string (required) - ThunderID server base URL"
}
```

### ThunderID Application Setup

Before running the app, ensure your ThunderID application is configured with:
...
- Ensure ThunderID server is running and accessible at the configured base URL
- Verify the client ID is correct
- Check that redirect URLs are properly configured in ThunderID

**Issue**: "Invalid client" error
- Double-check the `clientId` in your `runtime.json`
- Ensure the application exists in ThunderID and is enabled

**Issue**: CORS errors
- Add your application URL to "Allowed Origins" in ThunderID's `deployment.yaml`:
  ```yaml
  cors:
    allowed_origins:
...
1. **SDK Provider Setup**: The app wraps components with `AsgardeoProvider` configured with ThunderID's base URL and client ID
