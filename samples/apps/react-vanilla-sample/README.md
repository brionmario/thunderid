...
This sample React application demonstrates integrating ThunderID authentication into your application. It supports two authentication approaches:

- **Native Flow**: App-native authentication using ThunderID's flow orchestration API
...
For app-native authentication using ThunderID's flow API:
...
|----------|-------------|
| `flowEndpoint` | ThunderID's flow orchestration endpoint |
| `applicationsEndpoint` | ThunderID's applications API endpoint |
| `redirectBasedLogin` | Set to `false` for native flow |
| `applicationID` | The application ID registered in ThunderID (obtained during ThunderID setup) |
...
| `redirectUri` | Callback URL where ThunderID redirects after authentication |
...
For HTTPS support, copy the SSL certificates from your ThunderID distribution to the project root:
